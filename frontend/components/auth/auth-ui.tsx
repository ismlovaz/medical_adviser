...
export function AuthAlert({ message }: { message: string }) {
    return (
        <div className="mb-6 flex items-start gap-3 rounded-lg bg-green-50 p-4 text-sm text-green-800 border border-green-100">
...
export function GoogleSignInButton({ onClick, isLoading }: { onClick?: () => void, isLoading?: boolean }) {
    ...
    return (
        <Button
            variant="outline"
            type="button"
            className="w-full text-slate-700 font-normal"
            onClick={onClick}
            disabled={isLoading}
        >
